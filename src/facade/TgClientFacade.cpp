...
std::string TgClientFacade::auth_state_to_string(ITgClient::AuthState state) {
    ...
    switch (state) {
    case AuthState::WaitingPhone: return "WaitingPhone";
    case AuthState::WaitingCode: return "WaitingCode";
    case AuthState::Ready: return "Ready";
    case AuthState::LoggingOut: return "LoggingOut";
...
int TgClientFacade::handle_get_chats(int limit) {
    ...
    try {
        ...
        for (size_t i = 0; i < chats.size(); ++i) {
            ...
            const std::string label = label_controller_
                                          ? label_controller_->label_for_chat_id(chat.chatId)
                                          : std::string{};
...
int TgClientFacade::handle_chat_info(const std::string& chat_id) {
    ...
    try {
        const std::string resolved_id = label_controller_
                                            ? label_controller_->resolve_chat_id(chat_id)
                                            : chat_id;
...
int TgClientFacade::handle_history(const std::string& chat_id, int limit) {
    ...
    try {
        ...
        for (const auto& msg : messages) {
            std::cout << "[" << chats_controller_->get_chat_title(msg.sender) << "] " << "(" << msg.messageID << "): "
                << msg.text << "\n";
        }
